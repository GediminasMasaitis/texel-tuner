...
static Trace eval(Position& pos) {
    ...
    for (int c = 0; c < 2; ++c) {
        ...
        if (c == 0) {
            // Tempo bonus for the side to move (per-side-to-move term)
            TraceIncr(tempo);
        }
        ...
        for (int p = Pawn; p <= King; ++p) {
            ...
            while (copy) {
                ...
                if (p == Pawn && !(in_front & no_passers)) {
                    ...
                    // King distance to the square in front of the passer
                    for (int i = 0; i < 2; ++i) {
                        const int king_sq = lsb(pos.colour[i] & pos.pieces[King]);
                        const int rd = abs(king_sq / 8 - rank - 1);
                        const int fd = abs(king_sq % 8 - file);
                        const int d = (rank - 1) * max(rd, fd);
                        score += passed_king_distance[i] * d;
                        TraceAdd(passed_king_distance[i], d);
                    }
                }

                const u64 mobility = get_mobility(sq, p /*== King ? Queen : p*/, &pos);

                // Threats: bonus for a piece attacking a higher-valued enemy
                // piece (victim strictly higher value, king excluded). Indexed
                // by [pair][c]; c keeps the side-to-move asymmetry (c == 0 means
                // the attacker is to move, so the threat is more likely won).
                {
                    const u64 attacks = p == Pawn ? (ne(piece_bb) | nw(piece_bb)) : mobility;
                    int ti = (p - 1) * (10 - p) / 2;
                    for (int pv = p + 1; pv <= Queen; pv++, ti++) {
                        // Count the threat twice: the tuned value then lands at
                        // half magnitude, doubling the headroom before the int8
...
static void print_parameters_tapered(const parameters_t& parameters)
{
    ...
    for (auto i = 0; i < 2; i++)
    {
        ...
        print_array_tapered(ss, parameters, index, phase, "threats", 20);
